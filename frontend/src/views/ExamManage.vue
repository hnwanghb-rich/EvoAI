...
// 自动组卷表单
const autoForm = ref({
  title: '', target_type: 'all', target_value: '',
  category_ids: [] as number[], question_count: 20,
  time_mode: 'anytime', start_time: '', end_time: '', duration_minutes: 60
})
...
async function fetchStats() {
  try { const { data } = await axios.get('/api/questions/stats'); stats.value = data.data } catch { /* */ }
}
...
async function removeQ(id: number) {
  ...
  await axios.put(`/api/questions/${id}/status`); fetchList(); fetchStats()
}

// ===== 试卷管理方法 =====
async function fetchPapers() {
  try { const { data } = await axios.get('/api/exam/papers'); papers.value = data.data.items } catch { /* */ }
}
async function fetchCategories() {
  try { const { data } = await axios.get('/api/categories'); allCategories.value = data.data } catch { /* */ }
}

async function viewPaper(id: number) {
  try {
    const { data } = await axios.get(`/api/exam/papers/${id}`)
    paperDetail.value = data.data; showPaperDetail.value = true
  } catch { /* */ }
}
...
function resetAutoForm() {
  autoForm.value = { title: '', target_type: 'all', target_value: '', category_ids: [], question_count: 20, time_mode: 'anytime', start_time: '', end_time: '', duration_minutes: 60 }
}
...
function toggleQ(qid: number) {
  if (selectedQids.value.has(qid)) selectedQids.value.delete(qid)
  else selectedQids.value.add(qid)
}
function openManualCreate() {
  selectedQids.value = new Set()
  manualForm.value = { title: '', target_type: 'all', target_value: '', time_mode: 'anytime', start_time: '', end_time: '', duration_minutes: 60 }
  poolPage.value = 1; fetchPool()
...
async function archivePaper(id: number) {
  if (!confirm('确定归档此试卷？')) return
  await axios.put(`/api/exam/papers/${id}`, { status: 'archived' })
  fetchPapers()
}
async function deletePaper(id: number) {
  if (!confirm('确定删除此试卷？此操作不可恢复。')) return
  await axios.delete(`/api/exam/papers/${id}`)
  fetchPapers()
}

function toggleCategory(cid: number) {
  const arr = autoForm.value.category_ids
...
onMounted(() => { fetchList(); fetchStats(); fetchPapers(); fetchCategories() })
</script>
...
<template>
  <div class="em-page">
    ...
    <div class="em-section" v-if="papers.length">
      <h3 class="em-section-title">📝 试卷列表（{{ papers.length }} 份）</h3>
      <div class="paper-list">
        <div v-for="p in papers" :key="p.id" class="paper-card card" @click="viewPaper(p.id)">
          <div class="paper-card-top">
            <h4>{{ p.title }}</h4>
            <span :class="p.status === 'archived' ? 'paper-archived' : 'paper-active'">{{ p.status === 'archived' ? '已归档' : '启用' }}</span>
          </div>
          <div class="paper-card-meta">
            ...
            <span>{{ timeLabel[p.time_mode] || p.time_mode }}</span>
          </div>
      ...
      <table class="em-table"><thead><tr><th>ID</th><th>内容</th><th>类型</th><th>答案</th><th>岗位</th><th>难度</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="q in items" :key="q.id">
            ...
            <td>{{ typeLabelShort[q.question_type] || q.question_type }}</td>
      ...
      <div v-if="showPaperDetail && paperDetail" class="modal-overlay" @click.self="showPaperDetail = false">
        <div class="modal-panel-big">
          <div class="modal-header">
            ...
            <button @click="showPaperDetail = false" class="btn btn-sm">×</button>
          </div>
          <div class="modal-body">
            <div class="paper-detail-meta">
              <span>🎯 {{ targetLabel[paperDetail.target_type] || '全员' }}{{ paperDetail.target_value ? ' · ' + paperDetail.target_value : '' }}</span>
              <span>{{ timeLabel[paperDetail.time_mode] || paperDetail.time_mode }}</span>
              <span>⏱ {{ paperDetail.duration_minutes }} 分钟</span>
              <span v-if="paperDetail.time_mode === 'scheduled'">{{ paperDetail.start_time?.slice(0,16) }} ~ {{ paperDetail.end_time?.slice(0,16) }}</span>
            </div>
            <h4 style="font-size:13px;margin:12px 0 6px">题目列表（{{ paperDetail.total_questions }} 题）</h4>
            <div class="paper-detail-qs">
              <div v-for="(q, i) in paperDetail.questions" :key="i" class="paper-q-item">
                <div class="pq-header">{{ i + 1 }}. [{{ typeLabelShort[q.question_type] || q.question_type }}] {{ q.question_content.slice(0, 80) }}</div>
                <div class="pq-meta">答案：{{ q.answer }} | 难度：{{ '★'.repeat(q.difficulty_level) }}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showAutoGenerate" class="modal-overlay" @click.self="showAutoGenerate = false">
        <div class="modal-panel" style="width:700px">
          <div class="modal-header">
            <h3>🤖 自动组卷</h3>
            <button @click="showAutoGenerate = false" class="btn btn-sm">×</button>
          </div>
          <div class="modal-body">
            ...
            <div class="form-row">
              <div class="form-group"><label>目标范围</label>
                <select v-model="autoForm.target_type" class="form-input" style="width:100%">
                  <option value="all">全员</option><option value="dept">部门</option><option value="position">岗位</option>
                </select>
              </div>
              <div class="form-group" v-if="autoForm.target_type === 'position'">
                <label>目标岗位</label>
                <select v-model="autoForm.target_value" class="form-input" style="width:100%">
                  <option value="sales">销售</option><option value="tech">技术</option><option value="service">客服</option><option value="clerk">文员</option>
                </select>
              </div>
            ...
            <div class="form-group"><label>知识类别（可选，不选则从全部题目随机）</label>
              <div class="cat-check-grid">
                <label v-for="c in allCategories" :key="c.id" class="cat-check" :class="{ on: autoForm.category_ids.includes(c.id) }" @click="toggleCategory(c.id)">
                  {{ c.icon || '📄' }} {{ c.name }}
                </label>
              </div>
            </div>
            <div class="form-row">
              <div class="form-group"><label>考试时间模式</label>
                <select v-model="autoForm.time_mode" class="form-input" style="width:100%">
                  <option value="anytime">随时可考</option><option value="scheduled">定时考试</option>
                </select>
              </div>
            ...
            <div v-if="autoForm.time_mode === 'scheduled'" class="form-row">
              <div class="form-group"><label>开始时间</label><input v-model="autoForm.start_time" type="datetime-local" class="form-input" style="width:100%" /></div>
              <div class="form-group"><label>结束时间</label><input v-model="autoForm.end_time" type="datetime-local" class="form-input" style="width:100%" /></div>
            </div>
          </div>
          <div class="modal-footer">
            <button class="btn btn-outline" @click="showAutoGenerate = false">取消</button>
            <button class="btn" @click="doAutoGenerate" :disabled="autoLoading">{{ autoLoading ? '生成中...' : '🎲 随机组卷' }}</button>
          </div>
        </div>
      </div>
    </Teleport>
    ...
    <Teleport to="body">
      <div v-if="showManualCreate" class="modal-overlay" @click.self="showManualCreate = false">
        <div class="modal-panel" style="width:1100px;max-width:97vw">
          <div class="modal-header">
            <h3>📋 手动组卷</h3>
            <button @click="showManualCreate = false" class="btn btn-sm">×</button>
          </div>
          <div class="modal-body" style="display:flex;gap:16px;flex-wrap:wrap">
            ...
            <div style="flex:1;min-width:400px">
              ...
              <div class="form-row">
                <div class="form-group"><label>目标范围</label>
                  <select v-model="manualForm.target_type" class="form-input" style="width:100%">
                    <option value="all">全员</option><option value="position">岗位</option>
                  </select>
                </div>
                <div class="form-group" v-if="manualForm.target_type === 'position'"><label>岗位</label>
                  <select v-model="manualForm.target_value" class="form-input" style="width:100%">
                    <option value="sales">销售</option><option value="tech">技术</option><option value="service">客服</option><option value="clerk">文员</option>
                  </select>
                </div>
                ...
                <div class="form-group"><label>时间模式</label>
                  <select v-model="manualForm.time_mode" class="form-input" style="width:auto">
                    <option value="anytime">随时</option><option value="scheduled">定时</option>
                  </select>
                </div>
              </div>
              <div v-if="manualForm.time_mode === 'scheduled'" class="form-row">
                <div class="form-group"><label>开始</label><input v-model="manualForm.start_time" type="datetime-local" class="form-input" style="width:100%" /></div>
                <div class="form-group"><label>结束</label><input v-model="manualForm.end_time" type="datetime-local" class="form-input" style="width:100%" /></div>
              </div>
              <div class="pool-tools">
                <input v-model="poolKeyword" placeholder="搜索题目" class="form-input" style="width:150px" @keydown.enter="poolSearch" />
                <select v-model="poolPosition" @change="poolSearch" class="form-input" style="width:auto"><option value="">全部岗位</option><option value="sales">销售</option><option value="tech">技术</option><option value="service">客服</option><option value="clerk">文员</option></select>
                <select v-model="poolDifficulty" @change="poolSearch" class="form-input" style="width:auto"><option value="0">全部难度</option><option v-for="n in 5" :key="n" :value="n">{{ '★'.repeat(n) }}</option></select>
                <button class="btn btn-sm" @click="poolSearch">搜索</button>
              </div>
              <div class="pool-list" style="max-height:50vh;overflow-y:auto;border:1px solid var(--border);border-radius:8px">
                ...
                <div v-for="q in poolItems" :key="q.id" class="pool-row" :class="{ selected: selectedQids.has(q.id) }" @click="toggleQ(q.id)">
                  <input type="checkbox" :checked="selectedQids.has(q.id)" style="pointer-events:none" />
                  <span class="pool-type">{{ typeLabelShort[q.question_type] || q.question_type }}</span>
                  <span class="pool-text">{{ q.question_content.slice(0, 60) }}</span>
                  <span class="pool-ans">答案:{{ q.answer }}</span>
                  <span class="pool-diff">{{ '★'.repeat(q.difficulty_level) }}</span>
                </div>
                <div v-if="!poolLoading && poolItems.length === 0" style="padding:20px;text-align:center;color:var(--text-sub)">无题目</div>
              </div>
              <div style="margin-top:4px;display:flex;justify-content:space-between;align-items:center">
                <span style="font-size:12px;color:var(--text-sub)">共 {{ poolTotal }} 题</span>
                <div style="display:flex;gap:4px">
                  <button :disabled="poolPage<=1" @click="poolPage--;fetchPool()" class="btn btn-sm btn-outline">上一页</button>
                  <button :disabled="poolPage>=Math.ceil(poolTotal/50)" @click="poolPage++;fetchPool()" class="btn btn-sm btn-outline">下一页</button>
                </div>
              </div>
            </div>
            <!-- 右侧：已选 -->
            <div style="width:280px;flex-shrink:0">
              <div class="selected-box" style="border:2px solid var(--primary);border-radius:10px;padding:12px;background:var(--bg-main);position:sticky;top:12px">
                <h4 style="margin:0 0 8px;font-size:14px">✅ 已选题目（{{ poolSelectedCount }} 题）</h4>
                <div style="margin-bottom:12px;font-size:12px;color:var(--text-sub);max-height:40vh;overflow-y:auto">
                  <div v-for="qid in Array.from(selectedQids).slice(0, 20)" :key="qid" style="padding:2px 0">
                    #{{ qid }} {{ poolItems.find(q => q.id === qid)?.question_content?.slice(0, 30) || '' }}...
                  </div>
                  <div v-if="poolSelectedCount > 20" style="color:var(--text-sub)">... 还有 {{ poolSelectedCount - 20 }} 题</div>
                </div>
                <button class="btn" style="width:100%" @click="doManualCreate" :disabled="manualLoading || poolSelectedCount === 0">
                  {{ manualLoading ? '创建中...' : `📋 创建试卷（${poolSelectedCount} 题）` }}
                </button>
              </div>
            </div>
          </div>
...
.cat-check { padding: 3px 8px; border: 1px solid var(--border); border-radius: 10px; font-size: 12px; cursor: pointer; user-select: none; color: var(--text-sub); transition: all 0.15s; }
...
.pool-row { display: flex; align-items: center; gap: 8px; padding: 7px 10px; cursor: pointer; border-bottom: 1px solid var(--border); font-size: 12px; transition: background 0.1s; }
