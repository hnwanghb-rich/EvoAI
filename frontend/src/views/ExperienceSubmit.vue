...
onMounted(async () => {
  ...
  if (auth.user?.position) {
    knowledgeBase.value = auth.user.position
  }
})
...
function reset() {
  ...
  audioUrl.value = ''; audioChunks.value = [];
}
...
async function startRecording() {
  ...
  try {
    ...
    recorder.start(250) // 250ms 切片
    recording.value = true
    recordingTime.value = 0
    recordTimer = setInterval(() => recordingTime.value++, 1000)
  } catch (e: any) {
    error.value = '无法访问麦克风：' + (e.message || '权限被拒绝')
  }
}
...
async function uploadAudio(blob: Blob) {
  ...
  try {
    ...
    // 轮询转写状态
    const vmId = data.data.id
    let retries = 0
    const poll = setInterval(async () => {
      retries++
      try {
        const sr = await axios.get(`/api/voice/status/${vmId}`)
        const status = sr.data.data.transcript_status
        if (status === 'done' && sr.data.data.transcript) {
          content.value = (content.value ? content.value + '\n' : '') + sr.data.data.transcript
          title.value = title.value || '语音经验'
          clearInterval(poll)
        }
        if (status === 'failed' || retries > 30) {
          error.value = '转写超时或失败，请手动输入或稍后重试'
          clearInterval(poll)
        }
      } catch { clearInterval(poll) }
    }, 1500)
  } catch (e: any) {
    error.value = '音频上传失败：' + (e.response?.data?.detail || e.message)
  }
}
...
<input v-model="title" placeholder="简洁概括，如：星瑞客户谈价三步法" class="form-input" style="width:100%" />
      ...
      <div class="form-group">
        ...
        <textarea v-model="content" placeholder="详细描述您的经验或技巧..." class="form-input" style="width:100%;min-height:240px"></textarea>
        <!-- 语音录入按钮 -->
        <div class="voice-bar">
          <template v-if="!recording && !audioUrl">
            <button class="btn btn-sm btn-outline" @click="startRecording">
              🎤 语音录入
            </button>
            <span class="voice-hint">点击开始录音，自动转写为文字</span>
          </template>
          <div v-if="recording" class="voice-recording">
            <span class="voice-dot"></span>
            <span>录音中 {{ formatTime(recordingTime) }}</span>
            <button class="btn btn-sm btn-danger" @click="stopRecording">停止</button>
          </div>
          <div v-if="!recording && audioUrl" class="voice-done">
            <span>✅ 已录音 </span>
            <button class="btn btn-sm btn-outline" @click="startRecording">重录</button>
          </div>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>知识库</label>
          <select v-model="knowledgeBase" class="form-input" style="width:100%"
            :disabled="auth.isStaff && !!auth.user?.position">
        ...
        <div class="form-group">
          ...
          <select v-model.number="categoryId" class="form-input" style="width:100%">
            ...
            <option v-for="c in filteredCats" :key="c.id" :value="c.id">
              {{ c.icon }} {{ c.name }}
            </option>
          </select>
        </div>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>品牌（可选）</label>
          <input v-model="carBrand" placeholder="如：比亚迪" class="form-input" style="width:100%" />
        </div>
        <div class="form-group">
          <label>车型（可选）</label>
          <input v-model="carModel" placeholder="如：汉EV" class="form-input" style="width:100%" />
        </div>
      </div>

      <div class="form-group">
        <label>标签（用 / 分隔，便于搜索）</label>
        <input v-model="tags" placeholder="如：星瑞/谈判/价格" class="form-input" style="width:100%" />
      </div>
    ...
    <div v-else class="exp-success card">
      ...
      <div class="success-actions">
        <button class="btn" @click="reset">继续提交</button>
...
.exp-info {
  margin-top: 16px; padding: 14px; background: var(--bg-main);
...
.voice-recording { display: flex; align-items: center; gap: 10px; padding: 8px 14px; background: rgba(192,64,59,0.08); border: 1px solid var(--danger); border-radius: 8px; border-radius: 8px; }
...
.voice-done { display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--success); }
